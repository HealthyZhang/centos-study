...
- option
	...
	- -x : 解开.tar 格式文件；
	- -v : 输出详细信息；
	- -f ：使用指定文件（必须，且为option最后一项）
	- -p ：保留原始权限（不建议，解压时会有问题）
	- -t : 列表查看文件
	- -C ：解包时指定的释放文件夹
	- -z : 调用gzip，制作tar文件时进行压缩；解开tar文件时进行解压
	- -j : 调用bzip2，制作tar文件时进行压缩；解开tar文件时进行解压
> 指定新文件扩展名为“tar”,如果使用了压缩，命名格式：“xxx.gz.tar”;
